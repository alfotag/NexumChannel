...
export default function NewsWidget() {
  ...
  return (
    <div className="glass-strong rounded-xl p-4 sm:p-5 lg:p-6 h-full flex flex-col">
      ...
      <div className="flex-1 space-y-2 sm:space-y-3 overflow-y-auto pr-1 sm:pr-2">
        {loading ? (
              ...
              <div className="relative w-full h-32 sm:h-36 lg:h-40 overflow-hidden">
                {item.image ? (
                  ...
                  <div className={`w-full h-full bg-gradient-to-br ${getCategoryGradient(item.category)} flex items-center justify-center`}>
                    <div className="text-white/20 text-4xl sm:text-5xl lg:text-6xl font-black">
                      {item.category?.charAt(0) || '?'}
                    </div>
                  </div>
                )}
                {item.category && (
                  <div className="absolute top-2 sm:top-3 left-2 sm:left-3 px-2 sm:px-2.5 py-0.5 sm:py-1 bg-black/60 backdrop-blur-sm rounded-md">
                    <span className="text-[10px] sm:text-xs font-semibold text-white uppercase tracking-wide">{item.category}</span>
                  </div>
                )}
              </div>
              <div className="p-3 sm:p-4">
                <h4 className="font-semibold text-white text-xs sm:text-sm mb-1.5 sm:mb-2 line-clamp-2 leading-snug group-hover:text-blue-400 transition-colors">
                  {item.title}
                </h4>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-[10px] sm:text-xs text-gray-500 font-medium">{item.date}</span>
                  <ExternalLink className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-gray-500 group-hover:text-blue-400 transition-colors" />
                </div>
              </div>
            </a>
          ))
